...
import { checkInTrainingRegistrationAction, createTrainingRegistrationAction, getTrainingRegistrations, updateTrainingRegistrationAction } from "@/lib/features/training/actions/training-registration.actions";
...
type TrainingRegistrationWithRelations = any;
...
const manualRegistrationSchema = z.object({
	...
	externalRegistrationId: z.string().optional(),
});
...
interface TrainingEngagementRegistrationsProps {
	engagementId: string;
	pageSize?: number;
	page?: number;
}

export function TrainingEngagementRegistrations({ engagementId, pageSize: propPageSize = 20, page: propPage = 1 }: TrainingEngagementRegistrationsProps) {
	...
	const loadRegistrations = useCallback(
		async (currentSearch?: string, currentStatus?: string) => {
			try {
				...
				const result = await getTrainingRegistrations(engagementId, { skip, take: effectivePageSize }, { search: currentSearch, status: currentStatus });
				if (result.success && result.data) {
					// Handle paginated response
					if (result.data && typeof result.data === "object" && "registrations" in result.data) {
						let paginatedData = result.data as { registrations: TrainingRegistrationWithRelations[]; total: number };
						setRegistrations(paginatedData.registrations);
						setTotal(paginatedData.total);
					} else {
						// Handle non-paginated response (fallback)
						let regs = result.data as TrainingRegistrationWithRelations[];
	...
	}, [engagementId, page, effectivePageSize, searchTerm, statusFilterState, loadRegistrations]);

	const handleStatusChange = async (registrationId: string, newStatus: RegistrationStatus) => {
		try {
			const result = await updateTrainingRegistrationAction({ id: registrationId, status: newStatus });
			if (result.success) {
	...
	const handleCheckIn = async (registrationId: string) => {
		...
		try {
			const result = await checkInTrainingRegistrationAction(registrationId);
			if (result.success) {
				await loadRegistrations();
			} else {
	...
	const handleBulkAction = async (data: BulkActionFormData) => {
		...
		try {
			for (const registrationId of selectedRegistrations) {
				switch (data.action) {
					case "check_in":
						await checkInTrainingRegistrationAction(registrationId);
						break;
					case "cancel":
						await updateTrainingRegistrationAction({ id: registrationId, status: RegistrationStatus.CANCELLED });
						break;
				}
			}
	...
	const handleManualRegistration = async (data: ManualRegistrationFormData) => {
		try {
			const result = await createTrainingRegistrationAction({
				...data,
	...
	const exportRegistrations = () => {
		const csvData = registrations.map((registration) => ({
			Name: registration.user?.name || registration.contact?.firstName + " " + registration.contact?.lastName || "N/A",
			Email: registration.user?.email || registration.contact?.email || "N/A",
			Phone: registration.user?.phone || registration.contact?.phone || "",
			Status: registration.status,
			CheckInStatus: registration.checkedInAt ? CheckInStatus.CHECKED_IN : CheckInStatus.NOT_CHECKED_IN,
			RegisteredAt: format(new Date(registration.registeredAt), "yyyy-MM-dd HH:mm:ss"),
		...
		a.href = url;
		a.download = `training-registrations-${engagementId}.csv`;
		a.click();
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<h3 className="text-lg font-semibold">Training Registrations</h3>
								...
								<TableBody>
									{registrations.map((registration) => (
										<TableRow key={registration.id}>
											...
											<TableCell>
												<div className="flex items-center gap-3">
													<Avatar className="h-8 w-8">
														<AvatarImage src={registration.user?.avatar || registration.contact?.avatar} />
														<AvatarFallback>{registration.user?.name?.charAt(0) || registration.contact?.firstName?.charAt(0) || registration.contact?.lastName?.charAt(0) || "?"}</AvatarFallback>
													</Avatar>
													<div>
														<p className="font-medium">{registration.user?.name || (registration.contact?.firstName && registration.contact?.lastName ? `${registration.contact.firstName} ${registration.contact.lastName}` : "N/A")}</p>
														<p className="text-muted-foreground text-sm">{registration.user?.email || registration.contact?.email || "N/A"}</p>
														{(registration.user?.phone || registration.contact?.phone) && (
											...
											<TableCell>
												<div className="text-sm">{format(new Date(registration.registeredAt), "MMM dd, yyyy")}</div>
												<div className="text-muted-foreground text-xs">{format(new Date(registration.registeredAt), "HH:mm")}</div>
											</TableCell>
											<TableCell>
			...
			{total > effectivePageSize && (
				<Pagination
					...
					baseUrl={`/training-support/${engagementId}`}
					...
					<Form {...manualForm}>
						<form
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>External Registration ID (Optional)</FormLabel>
										<FormControl>
											<Input
