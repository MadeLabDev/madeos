...
export function TrainingAttendees({ initialRegistrations, initialTrainings, total, page, pageSize, search, trainingId, status, checkedIn }: { initialRegistrations: TrainingRegistrationWithRelations[]; initialTrainings: TrainingEngagementWithRelations[]; total: number; page: number; pageSize: number; search: string; trainingId: string; status: string; checkedIn: string }) {
	const router = useRouter();
	const [loading] = useState(false);
	const [searchInput, setSearchInput] = useState(search);
	const [trainingFilter, setTrainingFilter] = useState<string>(trainingId || "all");
	const [statusFilter, setStatusFilter] = useState<string>(status || "all");
	const [checkInFilter, setCheckInFilter] = useState<string>(checkedIn || "all");

	// Use props directly with useMemo
	const registrations = useMemo(() => initialRegistrations, [initialRegistrations]);
	const trainings = useMemo(() => initialTrainings, [initialTrainings]);
	...
	const handleSearch = () => {
		...
		router.push(`/training-support/attendees?${params.toString()}`);
	};

	const exportAttendees = () => {
		const csvData = registrations.map((registration) => ({
			Training: registration.trainingEngagement?.title || "Unknown Training",
			Name: getAttendeeName(registration),
			Email: getAttendeeEmail(registration),
			Phone: registration.contact?.phone || registration.user?.phone || "",
			TrainingType: registration.trainingEngagement?.trainingType || "N/A",
		...
		a.href = url;
		a.download = `training-attendees-${format(new Date(), "yyyy-MM-dd")}.csv`;
		a.click();
	...
	const getCheckInBadge = (registration: TrainingRegistrationWithRelations) => {
	...
	const getAttendeeName = (registration: TrainingRegistrationWithRelations) => {
		if (registration.user) {
			return registration.user.name || registration.user.email;
		}
		if (registration.contact) {
			return `${registration.contact.firstName} ${registration.contact.lastName}`.trim() || registration.contact.email;
		}
		return "Unknown Attendee";
	};

	const getAttendeeEmail = (registration: TrainingRegistrationWithRelations) => {
		if (registration.user) {
											...
											<TableCell>
												<div className="flex items-center gap-3">
													<Avatar className="h-8 w-8">
														...
														<AvatarFallback>{getAttendeeName(registration).charAt(0).toUpperCase()}</AvatarFallback>
													</Avatar>
													<div>
														<p className="font-medium">{getAttendeeName(registration)}</p>
														<p className="text-muted-foreground text-sm">{getAttendeeEmail(registration)}</p>
													</div>
												</div>
											</TableCell>
											<TableCell>
												<div>
													<p className="font-medium">{registration.trainingEngagement?.title || "Unknown Training"}</p>
													<p className="text-muted-foreground text-sm">{registration.trainingEngagement?.trainingType || "N/A"}</p>
												</div>
											</TableCell>
