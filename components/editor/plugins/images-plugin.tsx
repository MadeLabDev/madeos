...
export function InsertImageMediaPickerDialogBody({ onClick }: { onClick: (payload: InsertImagePayload) => void }) {
	const [isModalOpen, setIsModalOpen] = useState(false);

	const handleSelectMedia = (urls: string | string[]) => {
		const urlArray = Array.isArray(urls) ? urls : [urls];
		// Insert multiple images
		urlArray.forEach((url, index) => {
			// Add a small delay between insertions to avoid conflicts
			setTimeout(() => {
				onClick({ altText: `Uploaded image ${index + 1}`, src: url });
			}, index * 100);
		});
		...
		<div className="grid gap-4 py-4">
			<div className="space-y-4">
				<div>
					...
					<p className="text-muted-foreground mt-1 text-sm">Select one or more images from your media library or upload new ones</p>
				</div>
				<Button
					type="button"
					variant="outline"
					...
					className="w-full"
					data-test-id="image-modal-media-picker-btn">
					Open Media Library
				</Button>
			</div>
			...
			<MediaPickerModal
				...
				title="Select Images"
				description="Choose one or more images to insert into the editor"
			/>
		</div>
	);
}
	...
	return (
		<Tabs defaultValue="url">
			...
			<TabsContent value="media">
				<InsertImageMediaPickerDialogBody onClick={onClick} />
			</TabsContent>
		</Tabs>
	);
}
